...
CUSTNAME, CNIC,NTN,
 ...
 ADDRESS,              
  ...
  SUM(GROSS_AMT) AS GROSS_AMT,
   ...
   SUM(GROSS_AMT)- SUM(DISCOUNT_AMT) AS TAXABLE_AMT, 
   NVL(TAX_P,0),
  nvl(SUM(TAXAMOUNT),0) TAXAMOUNT,
  SUM(GROSS_AMT)- SUM(DISCOUNT_AMT)+nvl(SUM(TAXAMOUNT),0)  AS NET_AMT
  ,proprietor 
  FROM
(SELECT DISTINCT 
  ...
  --ROUND(SUM((SALE_DETAIL.GROSS_AMT-nvl(SALE_DETAIL.DISCOUNT_AMT,0))*NVL(SALE_DETAIL.TAX_P,0)/100),2) ADV_TAX,
  ...
  SUM(SALE_DETAIL.NET_AMT) AS NET_AMT,SALE_DETAIL.TAX_P,SUM(TAX_AMT) TAXAMOUNT
...
GROUP BY SALE_MASTER.CUSTOMERID,CUSTOMERS.CNIC,CUSTOMERS.NTN,CUSTOMERS.ORGNAME,CUSTOMERS.ADDRESS,
         ...
         PRODUCTS.PRODNAME,SALE_DETAIL.DISCOUNT_P,PRODUCTS.UNIT,PRODUCTS.UNITPRICE,SALE_DETAIL.TAX_P 
UNION ALL
SELECT DISTINCT 
...
/*SUM(RETURN_DETAIL.GROSS_AMT-
                        (RETURN_DETAIL.GROSS_AMT-(RETURN_DETAIL.GROSS_AMT*NVL(RETURN_DETAIL.TAX_P,0)/100) )
                         *NVL(RETURN_DETAIL.TAX_P,0)/100
)
* -1 AS GROSS_AMT,*/
SUM(RETURN_DETAIL.GROSS_AMT)*-1 AS GROSS_AMT,
--ROUND(SUM((RETURN_DETAIL.GROSS_AMT-nvl(RETURN_DETAIL.DISCOUNT_AMT,0))*NVL(RETURN_DETAIL.TAX_P,0)/100),2)* -1 AS  ADV_TAX, 
...
SUM(RETURN_DETAIL.NET_AMT) -1 AS NET_AMT
,RETURN_DETAIL.TAX_P
,SUM(TAX_AMT)*-1 TAXAMOUNT
...
GROUP BY RETURN_MASTER.CUSTOMERID, CUSTOMERS.CNIC,CUSTOMERS.NTN,
  ...
  PRODUCTS.UNIT,PRODUCTS.UNITPRICE,RETURN_DETAIL.TAX_P)
GROUP BY CUSTOMERID, CNIC,NTN,CUSTNAME,ORGNAME,ADDRESS,REGIONNAME,NVL(TAX_P,0), proprietor
ORDER BY 1,4,5

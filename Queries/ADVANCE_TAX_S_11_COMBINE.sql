...
CUSTNAME, CNIC,NTN,
 ...
 ADDRESS,              
  REGIONNAME, 
  ROUND(SUM(GROSS_AMT),2) AS GROSS_AMT,
   SUM(DISCOUNT_AMT) AS DISCOUNT_AMT, 
  ROUND(SUM(NET_AMT),2)  AS NET_AMT,
  NVL(TAX_P,0),
  ROUND(SUM(NET_AMT*NVL(TAX_P,0)/100),2) ADV_TAX,proprietor FROM*/
  
  SELECT DISTINCT 
NTN,CNIC,proprietor ,              
  REGIONNAME,ADDRESS,'' "TAX PAYER STATUS" ,
  CUSTOMERID,
CUSTNAME, 
  -- ORGNAME, 
  ...
  NVL(TAX_P,0),
  ROUND(SUM(NET_AMT*NVL(TAX_P,0)/100),2) ADV_TAX FROM
(SELECT DISTINCT 
...
UNION ALL
SELECT DISTINCT 
  RETURN_MASTER.CUSTOMERID,  CUSTOMERS.CNIC,CUSTOMERS.NTN,
CUSTOMERS.ORGNAME CUSTNAME, 
  RETURN_MASTER.CUSTOMERID||' - '||CUSTOMERS.ORGNAME AS ORGNAME, 
               CUSTOMERS.ADDRESS,CUSTOMERS.proprietor,
